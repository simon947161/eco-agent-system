param(
    [ValidateSet("Plan", "Retrieve", "Derive", "BuildProject", "Verify", "Open")]
    [string]$Action = "Plan",
    [string]$OsgeoRoot = "D:\"
)
...
$projectPath = Join-Path $workspaceRoot "project\Cooma_Spatial_Foundation_v0_4_integrated.qgz"
$pythonPath = Join-Path $OsgeoRoot "bin\python.exe"
$qgisLauncher = Join-Path $OsgeoRoot "bin\qgis-ltr.bat"

if ($Action -eq "Open") {
    if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
        throw "Integrated QGIS project not found: $projectPath"
    }
...
& $pythonPath -u -m cczps_lite.qgis_local_spatial_foundation.integrated_pack $actionMap[$Action] --osgeo-root $OsgeoRoot
if ($LASTEXITCODE -ne 0) {
    throw "QGIS Cooma integrated experience $($actionMap[$Action]) failed with exit code $LASTEXITCODE"
}
