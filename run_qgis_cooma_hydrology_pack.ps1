...
$projectPath = Join-Path $workspaceRoot "project\Cooma_Spatial_Foundation_v0_3_hydrology.qgz"
$pythonPath = Join-Path $OsgeoRoot "bin\python.exe"
$qgisLauncher = Join-Path $OsgeoRoot "bin\qgis-ltr.bat"

if ($Action -eq "Open") {
    if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
        throw "Hydrology QGIS project not found: $projectPath"
    }
...
& $pythonPath -u -m cczps_lite.qgis_local_spatial_foundation.hydrology_pack $actionMap[$Action] --osgeo-root $OsgeoRoot
if ($LASTEXITCODE -ne 0) {
    throw "QGIS Cooma hydrology pack $($actionMap[$Action]) failed with exit code $LASTEXITCODE"
}
