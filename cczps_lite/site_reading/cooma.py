...
def validate_request(r:dict[str,Any])->None:
 ...
 if not isinstance(r.get("temporal_context"),dict) or not {"assessment_period","valid_until","validity_basis","expiry_conditions"}<=r["temporal_context"].keys(): raise SiteReadingError("Temporal context is incomplete")
...
def contract_envelope(req,*,issued_at,object_id,object_type):
 return {"object_id":object_id,"object_type":object_type,"schema_version":"0.1","revision_id":req["revision_id"],"parent_revision":req["parent_revision"],"supersedes":[],"created_at":issued_at,"issued_at":issued_at,"author_role":"CODEX_EXECUTION_AGENT","place_scope":req["place"],"spatial_boundary_id":"COOMA-LOCALITY-PLUS-10KM-ORIENTATION_AND-STATION-410033","spatial_boundary_reference":"Request spatial_context_ids; gauge evidence remains station-bounded","time_scope":req["temporal_context"]["assessment_period"],"evidence_cutoff":"2026-07-27","intended_use":req["decision_use"],"prohibited_uses":["A1_OR_HIGHER","CURRENT_FLOW_OR_TREND","ENGINEERING","REGULATORY","PROCUREMENT","PUBLIC_WARNING"],"source_object_ids":req["evidence_object_ids"],"uncertainties":req["known_constraints"],"review_state":"PENDING_HUMAN_REVIEW","maximum_conclusion_level":"L2","maximum_intervention_class":"A0","valid_until":req["temporal_context"]["valid_until"],"stop_reasons":["attempted A1+ use","attempted current flow/trend claim","identity mismatch","professional gate failure"]}

def build_outputs(req,real,hydro,ha,hp,hr,*,issued_at):
 src=validate_inputs(req,real,hydro,ha,hp,hr); weather=src["COOMA-BOM-DWO-2026-07"]; outlook=src["BOM-ENSO-MONITORING-2026-07-14"]
 spatial=[{"spatial_context_id":sid,**SPATIAL_CONTEXT[sid]} for sid in req["spatial_context_ids"]]
 env={"skill_id":SKILL_ID,"skill_version":SKILL_VERSION,"request_id":req["request_id"],"revision_id":req["revision_id"],"parent_revision":req["parent_revision"],"issued_at":issued_at,"evidence_cutoff":"2026-07-27","valid_until":req["temporal_context"]["valid_until"],"validity_basis":req["temporal_context"]["validity_basis"],"expiry_conditions":req["temporal_context"]["expiry_conditions"],"maximum_intervention_class":"A0"}
 claims=[
  ...
  {"reasoning_type":"KNOWN_FROM_ADMITTED_EVIDENCE","statement":f"The admitted BoM archive records {outlook['parsed_metadata']['state'].replace('_',' ').title()} on {outlook['parsed_metadata']['archive_date']}; no local Cooma impact is inferred, and tree-to-leaf translation requires separately admitted local evidence and method.","evidence_ids":[outlook["source_id"]]},
 ...
 pc01={**contract_envelope(req,issued_at=issued_at,object_id=f"PC01-{req['revision_id']}",object_type="PC-01_SITE_OBSERVATION"),"schema_id":"climateos.planner.pc01.site_observation.v0.1","state":"EVIDENCE_REQUIRED","observation_status":"NONE_ADMITTED_FOR_THIS_READING","observation_method":None,"geometry":None,"observed_at":None,"limitations":["No direct human/instrument observation with time, method and geometry was admitted"]}
 pc02={**contract_envelope(req,issued_at=issued_at,object_id=f"PC02-{req['revision_id']}",object_type="PC-02_EVIDENCE_REQUEST"),"schema_id":"climateos.planner.pc02.evidence_request.v0.1","state":"OPEN","question_served":req["environmental_question"],"requested_objects":["Exact WaterNSW 410033 near-current response and receipt","Qualified hydrology review","Admitted local ecological/soil/land-cover observations"],"acceptable_source_class":"AUTHORITATIVE_AND_LAWFULLY_ADMITTED","network_limit":"SEPARATE_AUTHORIZATION_REQUIRED","cost_limit_aud":0,"fulfilment_test":"Identity, provenance, temporal/spatial fitness and review gates pass"}
 pc03={**contract_envelope(req,issued_at=issued_at,object_id=f"PC03-{req['revision_id']}",object_type="PC-03_PROFESSIONAL_REVIEW_GATE_REFERENCE"),"schema_id":"climateos.planner.pc03.professional_review_gate_reference.v0.1","state":"PENDING","domain":"HYDROLOGY","reference":"PR #115 / H1-H8 professional review card","evidence_version":hp["passport_id"],"review_questions":ha["human_review"]["review_questions"],"signature":None,"professional_signoff_simulated":False}
 pc04={**contract_envelope(req,issued_at=issued_at,object_id=f"PC04-{req['revision_id']}",object_type="PC-04_ENVIRONMENTAL_STATE"),"schema_id":"climateos.planner.pc04.environmental_state.v0.1","state":"UNKNOWN","state_variable":"CURRENT_COOMA_AND_MITTAGANG_HYDROLOGICAL_CONDITION","assertion_class":"UNKNOWN","value":None,"supporting_evidence":[ha["answer_id"]],"blocked_evidence":[hydro["run_id"]],"derivation":"No current-state derivation performed","confidence_basis":"Historical baseline reproducible; current evidence blocked"}
 pc13={**contract_envelope(req,issued_at=issued_at,object_id=f"PC13-{req['revision_id']}",object_type="PC-13_ACTION_PASSPORT"),"schema_id":"climateos.planner.pc13.action_passport.v0.1","action_passport_id":f"PC13-{req['revision_id']}","status":"READY_FOR_HUMAN_REVIEW","question":req["environmental_question"],"environmental_state_ids":[pc04["object_id"]],"permissible_claims":["Historical S0/L2 context exists","Current comparison remains blocked"],"recommended_consideration":"Preserve records, request missing evidence and prepare qualified review","permitted_a0_actions":["MONITORING_PREPARATION","EVIDENCE_REQUEST","RECORD_PRESERVATION","REVIEW_PREPARATION"],"prohibited_actions":["A1_OR_HIGHER","ENGINEERING","REGULATORY","PROCUREMENT","PUBLIC_WARNING"],"authority":{"accountable_human_role":"Founder or authorised reviewer","maximum_intervention_class":"A0"},"professional_review_gates":[pc03["object_id"]]}
 ...
 answer["validity_basis"]=env["validity_basis"]
 answer["expiry_conditions"]=env["expiry_conditions"]
...
Validity basis: {a['validity_basis']}. Expiry conditions: {', '.join(a['expiry_conditions'])}.
...
def run(output_root,*,request_path=None,repo_root=None,issued_at=None):
 ...
 artifact_root=_posix(Path(output_root),root) if Path(output_root).is_absolute() and Path(output_root).is_relative_to(root) else PurePosixPath(Path(output_root)).as_posix()
 rec={"schema_id":"climateos.cooma_site_reading.run_receipt.v0.1-r1","skill_id":SKILL_ID,"skill_version":SKILL_VERSION,"request_id":req["request_id"],"revision_id":req["revision_id"],"parent_revision":req["parent_revision"],"issued_at":stamp,"network_used":False,"artifact_root":artifact_root,"inputs":[_posix(req_path,root),*[_posix(p,root) for p in pin.values()]],"outputs":{k:PurePosixPath(p.relative_to(Path(output_root))).as_posix() for k,p in result.items()},"reading_digest":o["reading"]["content_digest"],"result":"BOUNDED_SITE_READING_PRODUCED","limitations":["ADMISSION_BLOCKED_MISSING_RAW_RESPONSE","NOT_COMPARABLE_YET","TREND_DEFERRED"]}
...
def verify_fixture(*,repo_root=None):
 root=Path(repo_root) if repo_root else Path(__file__).resolve().parents[2]
 request_path=root/"cczps_lite/input/cooma_site_reading_request_r1.json"
 request=json.loads(request_path.read_text(encoding="utf-8"))
 committed=root/"cczps_lite/output/cooma_site_reading_v0_1/runs"/request["revision_id"]
 if not committed.is_dir(): raise SiteReadingError("Committed R1 fixture directory is missing")
 with tempfile.TemporaryDirectory() as temp:
  generated=run(Path(temp)/"artifact_root",request_path=request_path,repo_root=root,issued_at=request["fixture_issued_at"])
  for key,path in generated.items():
   if key=="receipt": continue
   expected=committed/path.name
   if not expected.is_file() or path.read_bytes()!=expected.read_bytes(): raise SiteReadingError(f"Fixture verification mismatch: {path.name}")
  generated_receipt=json.loads(generated["receipt"].read_text(encoding="utf-8"))
  committed_receipt=json.loads((committed/"run_receipt.json").read_text(encoding="utf-8"))
  for field in ("reading_digest","revision_id","parent_revision","limitations","result"):
   if generated_receipt.get(field)!=committed_receipt.get(field): raise SiteReadingError(f"Fixture receipt mismatch: {field}")
 return {"status":"PASS","revision_id":request["revision_id"],"verified_files":len(generated)-1,"tracked_files_modified":False}
