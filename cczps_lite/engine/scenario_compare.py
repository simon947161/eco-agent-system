"""Run the CCZPS-Lite scenario comparison engine."""
...
from scenario_validation_pack import extend_validation_outputs  # noqa: E402
...
def evidence_for_scenario(scenario: dict, evidence_profile: dict) -> dict:
    ...
    keys = SCENARIO_EVIDENCE_PATHWAYS.get(scenario.get("scenario_type"), ())
    return {key: evidence[key] for key in keys if key in evidence}


def build_comparison_rows(
    scenarios: list[dict], evidence_profile: dict, context_records: list[dict]
) -> list[dict]:
    rows = []
...
def write_scenario_report(location: dict, scenarios: list[dict], rows: list[dict]) -> None:
    ...
    for heading, scenario in zip(headings, scenarios):
...
def _rank(value: str) -> int:
    return {"Low": 1, "Medium": 2, "High": 3, "Routine": 1, "Moderate": 2, "Critical": 3}.get(value, 0)


def _split(row: dict, field: str) -> list[str]:
    return [value.strip() for value in row[field].split(";") if value.strip()]


def _most_common(rows: list[dict], field: str) -> str:
    counts = Counter(value for row in rows for value in _split(row, field))
    if not counts:
...
def _names(rows: list[dict], field: str, value: str) -> str:
...
def write_governance_summary(rows: list[dict]) -> None:
    highest = max(rows, key=lambda row: row["risk_adjusted_score"])
    strongest = max(rows, key=lambda row: _rank(row["evidence_strength"]))
    weakest = min(rows, key=lambda row: _rank(row["evidence_strength"]))
    highest_review = max(rows, key=lambda row: (_rank(row["review_priority"]), -row["validation_score"]))
    highest_response = max(rows, key=lambda row: (_rank(row["response_priority"]), -row["validation_score"]))
    highest_implementation = max(
        rows,
        key=lambda row: (
            _rank(row["implementation_priority"]),
            _rank(row["urgency_level"]),
            -row["validation_score"],
        ),
    )
    human_review = ", ".join(
        row["scenario_name"] for row in rows if row["human_review_required"]
    ) or "None flagged by the current low-evidence rule"
    owner_counts = Counter(row["review_owner"] for row in rows)
    common_owner, owner_count = max(owner_counts.items(), key=lambda item: (item[1], item[0]))
    lines = [
        ...
        f"- {highest['scenario_name']} has the highest indicative risk-adjusted score ({highest['risk_adjusted_score']}).",
        f"- Recommendation class: {highest['recommendation_class']}.", "",
        "## Key Trade-Offs", "",
        ...
        f"- Scenarios requiring human review: {human_review}.", "",
        "## Differential Field Reading", "",
        "These gradients compare scenario scores with indicative representative context records only.", "",
        "## Forcing Layer Reading", "",
        f"- Most common candidate forcing: {_most_common(rows, 'forcing_candidates')}.", "",
        "## Validation Layer Runtime", "",
        f"- Strongest validation reading: {max(rows, key=lambda row: row['validation_score'])['scenario_name']}.",
        f"- Weakest validation reading: {min(rows, key=lambda row: row['validation_score'])['scenario_name']}.", "",
        "## Review Loop Reading", "",
        ...
        f"- Pathways held for evidence: {_names(rows, 'review_action', 'Hold and collect evidence')}.",
        f"- Pathways requiring technical review: {_names(rows, 'review_action', 'Escalate to technical review')}.",
        f"- Most common review owner: {common_owner} ({owner_count} scenario(s)).", "",
        ...
        f"- Most common response option: {_most_common(rows, 'response_options')}.", "",
        ...
        f"- Most common prioritised response: {_most_common(rows, 'prioritised_response')}.",
...
def main() -> None:
    ...
    rows = build_comparison_rows(
        scenarios, evidence_profile, differential_context.get("records", [])
    )
    write_comparison_csv(rows)
    write_scenario_report(location, scenarios, rows)
    write_governance_summary(rows)
    extend_validation_outputs()
    print("Generated CCZPS-Lite comparison and multi-scale validation outputs in cczps_lite/output")
