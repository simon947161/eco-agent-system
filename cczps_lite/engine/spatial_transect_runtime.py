...
def _copy_optional_fields(point: dict, output: dict) -> None:
    for field in ("direction_label", "direction_degrees", "distance_km", "elevation_m", "elevation_context", "source", "confidence", "status", "notes"):
...
def normalize_point(point: dict, expected_roles: tuple[str, ...], seen_ids: set[str]) -> tuple[dict, list[str]]:
    ...
    normalized = {
        ...
        "missing_data_status": ",".join(coordinate_issues) if coordinate_issues else "complete",
    }
...
def validate_transect(transect: dict) -> dict:
    ...
    return {
        "transect_id": transect.get("transect_id"),
        "scenario_id": transect.get("scenario_id"),
        "scenario_name": transect.get("scenario_name"),
        "core_location": normalized_core,
        "reference_points": normalized_references,
        "relationship_summary": {
            "reference_point_count": len(normalized_references),
            "configured_reference_roles": sorted(role for role, count in role_counts.items() if count > 0),
            "missing_data_points": missing_data,
            "relationship_inference": "not_performed",
        },
...
def build_spatial_transect_output(config: dict | None = None) -> dict:
    ...
    return {
        ...
        "runtime": "Spatial Context & Transect Runtime",
        "generated_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "provider_agnostic": True,
        "safety_boundary": SAFETY_BOUNDARY,
        "supported_point_roles": list(POINT_ROLES),
        "future_compatible_sources": list(config.get("future_compatible_sources") or FUTURE_COMPATIBLE_SOURCES),
        "output_notes": [
            "Reference points are consumed only when supplied by configuration.",
            "Direction, distance, and elevation values are preserved when supplied and are not inferred.",
            "Missing coordinates remain explicit in missing_data_status and validation output.",
        ],
        "spatial_transects": [validate_transect(transect) for transect in config.get("transects", [])],
    }


def render_markdown_report(output: dict) -> str:
    lines = [
        "# Spatial Context & Transect Runtime", "", f"Schema version: {output['schema_version']}", "", output["safety_boundary"], "",
        "## Supported Roles", "", ", ".join(output["supported_point_roles"]), "", "## Future Compatibility", "", ", ".join(output["future_compatible_sources"]), "", "## Transect Readings", "",
    ]
    for transect in output["spatial_transects"]:
        summary = transect["relationship_summary"]
        lines.extend([
            f"### {transect.get('scenario_name') or transect.get('scenario_id')}", "",
            f"- Transect ID: {transect.get('transect_id')}",
            f"- Scenario ID: {transect.get('scenario_id')}",
            f"- Core location: {transect['core_location'].get('name')}",
            f"- Validation status: {transect['validation']['status']}",
            f"- Configured roles: {', '.join(summary['configured_reference_roles']) or 'none'}",
            f"- Missing-data points: {', '.join(summary['missing_data_points']) or 'none'}",
            f"- Relationship inference: {summary['relationship_inference']}", "",
        ])
    return "\n".join(lines).rstrip() + "\n"


def write_spatial_transect_outputs(output: dict, output_path: Path = OUTPUT_PATH, report_path: Path = REPORT_PATH) -> None:
